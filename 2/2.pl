%Part one
occurencyRules(Min,Max, Char, Pw):-
    atom_chars(Pw,PwAsList),
    findall(C,(member(C,PwAsList),C = Char),Occ),
    length(Occ, N),
    N >= Min,
    N =< Max.
...
solvePartOne(Solution):-
    input(Input),
    findall(Pw, (member((Min, Max, Char, Pw), Input), occurencyRules(Min,Max,Char,Pw)), GoodPws),
    length(GoodPws, Solution).
    

%Part two

charAtOneOfTwoPositions(Position1,Position2,Char,Pw):-
    atom_chars(Pw,PwAsList),
    not(
        (
            nth1(Position1, PwAsList, Char),
            nth1(Position2, PwAsList, Char)
        )
    ),
    (nth1(Position1, PwAsList, Char) ; nth1(Position2, PwAsList, Char)).

solvePartTwo(Solution):-
    input(Input),
    findall(Pw, (
        member((Position1, Position2, Char, Pw), Input),
        charAtOneOfTwoPositions(Position1,Position2,Char,Pw)
        ), GoodPws),
    length(GoodPws, Solution).
